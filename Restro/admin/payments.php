...
if (isset($_GET['cancel'])) {
    $id = $_GET['cancel']; // Get the order ID from the URL
    $adn = "DELETE FROM  rpos_orders  WHERE  order_id = ?";//Deletes the specified order from the rpos_orders table using a prepared statement to prevent SQL injection.
    $stmt = $mysqli->prepare($adn);
    $stmt->bind_param('s', $id);// Bind the order ID parameter
    $stmt->execute();
    $stmt->close();
    if ($stmt) {
