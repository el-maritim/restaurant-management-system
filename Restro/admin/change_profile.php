...
include('config/checklogin.php');// Include the login check file to ensure the admin is logged in
check_login();
//Update Profile
if (isset($_POST['ChangeProfile'])) {
  $admin_id = $_SESSION['admin_id']; // Retrieve the admin ID from the session
  // Get the new name and email submitted from the form
  $admin_name = $_POST['admin_name'];
  $admin_email = $_POST['admin_email'];
  $Qry = "UPDATE rpos_admin SET admin_name =?, admin_email =? WHERE admin_id =?";
  $postStmt = $mysqli->prepare($Qry); // Prepare the statement to avoid SQL injection
  //bind paramaters
  ...
  if ($postStmt) {
    ...
    // Redirect to the dashboard and show a success message if the update is successful
  } else {
...
if (isset($_POST['changePassword'])) {
  ...
  if (isset($_POST['old_password']) && !empty($_POST['old_password'])) {
    // Hash the old password for secure comparison
  ...
  if (!$error) {
    ...
    if (mysqli_num_rows($res) > 0) {
      ...
      if ($old_password != $row['admin_password']) {
        ...
        //declare a varible which will be passed to alert function,,redirects you to dashboard
        if ($stmt) {
